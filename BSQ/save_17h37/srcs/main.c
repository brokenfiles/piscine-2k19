...
/*   Updated: 2019/08/21 17:33:29 by llaurent         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		send_to_algo(char *content, int ac, char **av,  t_map map)
{
	int	index;

	index = 0;
	if (get_map(1, -1, "", &map, content) != SUCCESS)
	{
	...
	{
		err(0, ac, MSG_ERR_CANNOT_READ_MAP);
		return (ERR_CANNOT_READ_MAP);
	}
	fill_map(&map);
	display_map(map);
...
int	main(int ac, char **av)
{
	...
	index = 1;
	if (ac < 2)
		if (send_to_algo(write_stdin(), ac, av, map) != SUCCESS)
			return (ERR_CANNOT_READ_MAP);
	while (index < ac)
	{
		if (get_map(1, -1, av[index], &map, "\0") != SUCCESS)
		{
			err(&index, ac, MSG_ERR_CANNOT_READ_MAP);
			continue;
		}
